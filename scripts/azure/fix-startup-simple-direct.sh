#!/bin/bash
# Simple direct startup - no extraction needed, Azure extracts zip automatically

RESOURCE_GROUP="${RESOURCE_GROUP:-healthforesight-rg}"
API_APP_NAME="${API_APP_NAME:-healthforesight-api-9016}"

echo "Setting simple direct startup command (no extraction)..."
echo ""
...
# Simple startup command that finds code where Azure puts it
# Azure automatically extracts the zip to /home/site/wwwroot
echo "2. Setting simple startup command (finds code in wwwroot)..."
STARTUP_CMD='cd /home/site/wwwroot && \
# Find where uepi_api actually is
...
if [ -n "$UEPI_DIR" ] && [ -d "$UEPI_DIR/uepi_api" ]; then \
    ...
    echo "Searching for uepi_api..."; \
    find . -name "uepi_api" -type d 2>/dev/null | head -5; \
...
python3 -m pip install --user uvicorn fastapi > /tmp/pip-install.log 2>&1; \
\
# Run uvicorn
echo "Starting uvicorn with PYTHONPATH=$PYTHONPATH"; \
python3 -m uvicorn uepi_api.main:app --host 0.0.0.0 --port 8000'
